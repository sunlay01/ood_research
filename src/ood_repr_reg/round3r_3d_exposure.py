...
from .round3r_3b_geometry import matrix_rank, subspace_basis
...
def source_design_span(source_states: list[MomentState | Array] | tuple[MomentState | Array, ...], reference_index: int = 0,
    ...
    basis = subspace_basis(contrasts)
    return {
        "reference": reference,
...
def exposed_response_basis(source_contrasts: Array, response_map: Array,
    ...
    basis = subspace_basis(image)
    return {"responses": image, "basis": basis, "rank": int(basis.shape[1]),
            "input_rank": matrix_rank(contrasts), "tolerance": float(tolerance)}
...
def quotient_dimensions(total_basis: Array, exposed_basis: Array, tolerance: float = 1e-9) -> dict[str, object]:
    total = subspace_basis(np.asarray(total_basis, dtype=float))
    ...
    rank_total = matrix_rank(total)
    rank_exposed = matrix_rank(exposed)
    ...
    return {"total_dimension": int(rank_total), "exposed_dimension": int(rank_exposed),
            ...
            "combined_rank": int(matrix_rank(combined)), "tolerance": float(tolerance)}


def source_reference_audit(source_states: list[MomentState | Array] | tuple[MomentState | Array, ...],
                           response_map: Array) -> dict[str, object]:
    first = source_design_span(source_states, 0)
    last = source_design_span(source_states, len(source_states) - 1)
    first_image = subspace_basis(np.asarray(response_map) @ first["contrasts"])
    last_image = subspace_basis(np.asarray(response_map) @ last["contrasts"])
