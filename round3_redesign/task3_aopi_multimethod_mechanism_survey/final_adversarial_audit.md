...
Q6. Spectral changes: largest top singular value reduction is `SPECTRAL_NORM_REG` (-1.367); tail/effective rank is highest under `SVB_ORTHDNN` (63.978); stable rank is highest under `SVB_ORTHDNN` (53.623).
Q7. Flatness changes: lowest Hessian top eigenvalue is `FISHR` (0.267), lowest trace is `SPECTRAL_NORM_REG` (0.030), and lowest SAM sharpness@0.05 is `SAM` (0.008).
Q8. OOD target accuracy: best target mean is `IRMv1` (0.669); worst target mean is `SPECTRAL_REG_2024` (0.102).
